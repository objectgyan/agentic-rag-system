...
| **Conversation context mgmt** | 🟡 **Weak** | Loads **all** history every turn — no window/summary/token budget. Long sessions blow context & cost. |
...
**Acceptance criteria:**
- [ ] History passed to the generator is bounded by a configurable token budget.
- [ ] Old turns are summarized, not dropped silently.
- [ ] A test proves a 50-turn conversation stays under budget and keeps early facts.
...
- **2026-07-05** — Roadmap captured. Built ① eval harness: `app/services/rag/evaluation/`
  ...
  generation faithfulness/completeness 1.0, answers correctly cited). Remaining for ①: write the
  end-to-end tutorial (with the metric definitions).
