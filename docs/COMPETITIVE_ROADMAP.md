...
| Scale (vectors / sparse / cache) | 🟢 Addressed | Item ⑤ done: HNSW ANN index verified; query **embedding cache** (Redis); **sparse search moved in-DB** — `content_tsv` generated column + GIN index + `ts_rank` (migration 006), replacing the in-Python BM25 that loaded ≤1000 rows. Eval-gated: recall@k/MRR/nDCG unchanged. |
...
- [x] Sparse search runs **in the DB**, not by loading 1000 rows into the app — `chunks.content_tsv` generated column + GIN index + `ts_rank`/`websearch_to_tsquery` (migration 006). **Eval-gated** with item ①'s harness: recall@5 / MRR / nDCG@5 all unchanged at 1.0 (precision within RRF noise) — no regression.
- [ ] *(stretch)* Semantic *answer* cache; batch/concurrent ingestion tuning.

**Status: ⑤ complete** — HNSW verified, embedding cache (5 tests), sparse-in-DB (eval-gated, no regression).
...
- **2026-07-05** — **Item ⑤ done** — HNSW ANN index verified; query embedding cache in Redis
  (`embedder.py`, fail-soft, 5 tests, verified vs real Redis); **sparse search moved in-DB** —
  migration 006 adds `chunks.content_tsv` (generated) + GIN index, `_sparse_search` rewritten to
  `ts_rank`/`websearch_to_tsquery`, removing the in-Python BM25 (numpy/rank-bm25). Eval-gated: recall@5
  / MRR / nDCG@5 unchanged at 1.0 vs BM25, no regression. Full suite 149 pass, ruff clean.
